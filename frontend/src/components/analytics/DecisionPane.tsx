...
const EffectNode = ({ data }) => {
  ...
  return (
    <div className={`px-4 py-3 rounded-lg border bg-gradient-to-br ${orderColors[data.order]}`}>
      <Handle type="target" position={Position.Top} className="!bg-white/30" />
      <div className="min-w-[200px]">
        <div className="flex items-center gap-2 mb-2">
          <div className={`w-2 h-2 rounded-full ${
            data.order === 1 ? 'bg-blue-500' :
            data.order === 2 ? 'bg-purple-500' :
            'bg-pink-500'
          }`} />
...
const FlowChart: React.FC<{ nodes: Node[]; edges: Edge[]; onNodesChange: any; onEdgesChange: any }> = ({
  nodes,
  edges,
  onNodesChange,
  onEdgesChange
...
const DecisionPane: React.FC<DecisionPaneProps> = ({ event, selectedDecision, onDecisionSelect }) => {
  ...
  React.useEffect(() => {
    if (selectedDecision) {
      const { nodes: newNodes, edges: newEdges } = createFlowElements(selectedDecision.effects);
      setNodes(newNodes);
      setEdges(newEdges);
    }
  ...
  const renderContent = () => {
    ...
    return (
      <div className="p-6">
        <h3 className="text-md font-medium text-white/90 mb-6">Potential Decisions</h3>
          ...
          {event.decisions?.map(decision => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <svg className="w-5 h-5 text-emerald-400/40 group-hover:text-emerald-400/60 transition-colors" 
          ...
          {selectedDecision && (
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 
                ...
                hover:border-white/20 flex items-center gap-2 group"
            >
        ...
        <div className="flex gap-4">
          <input
            ...
            className="flex-1 bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-white/90 
              placeholder:text-white/30 focus:outline-none focus:border-emerald-500/30"
          />
          <button
            className="px-4 py-2 bg-emerald-600/80 hover:bg-emerald-600 rounded-lg text-white/90 
