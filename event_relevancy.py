            ...
            try:
                with open(filename, 'r') as f:
                    ...
                    for event in file_events:
                        if code_upper in event.get("region_codes", []):
                            events.append(event)
...
def assess_events_relevancy_batch(events_batch: List[Dict[str, Any]], company_context: str, query: str) -> Dict[str, float]:
    ...
    for idx, event in enumerate(events_batch, start=1):
        prompt += (
            f"{idx}. Event ID: {event.get('event_id', '')}\n"
    ...
    try:
        response = openai.ChatCompletion.create(
            model="gemini-1206",  # Replace with the correct model if needed
            messages=[
                {"role": "system", "content": "You are an expert in assessing regulatory risk relevance, accounting for straightforward regulatory and supply chain risks, as well as more hidden tail risks."},
...
def get_relevant_events(company_context: str, country_codes: List[str], query: str, max_events: int) -> Dict[str, Any]:
    ...
    batches = batch_events(events, batch_size=10)
    ...
    result = {
        "relevant_event_ids": [e["event_id"] for e in selected_events],
